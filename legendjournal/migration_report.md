# Binary Options Trading OS Migration Report

## 1. Executive Summary

This report outlines the findings of a technical and structural audit performed on the existing Obsidian-based Trading OS repository. The current repository (`legendjournal`) was originally designed to accommodate both Forex and Binary Options trading, but acts primarily as a dual-system (or Forex-first) environment where `trade-type` distinguishes between "FOREX" and "BINARY".

The objective of this migration is to pivot the entire architecture into a dedicated **Binary Options Trading OS**, abandoning any unnecessary Forex baggage. The migration will restructure templates, dashboards, and system metadata to natively support Binary Options concepts (e.g., Fixed Expiry, CALL/PUT, Payout, Stake, and OTC algorithmic behavior) as the foundational model rather than an alternative `trade-type`.
...
- `00-OS/`: Core dashboards (e.g., Trading, Binary Options, Market, Strategy, Review) and system configuration.
...
- **Dual Support (Forex/Binary):** Currently, the system prompts for `trade-type` ("FOREX" or "BINARY") in the trade template. `Dashboard - Trading.md` acts as a generic (but heavily Forex-leaning) dashboard, while `Dashboard - Binary Options.md` attempts to filter specifically for BO trades (`WHERE trade-type = "BINARY"`).
...
*   **Dashboards (`00-OS/`)**:
    ...
    *   `Dashboard - Binary Options.md`: A separate view explicitly filtering for `trade-type = "BINARY"`. Evaluates Payout, Stake, and OTC Market Behaviors.
...
*   **Session Types (e.g., "London", "New York")**: Forex trading heavily revolves around specific global market opens. Binary Options (especially OTC on platforms like Quotex) is often traded 24/7 or relies on different session logic. `SES-TYPE-London.md` is a Forex paradigm.
...
*   **Research & Evidence (`03-Research/`, `08-Evidence/`)**: The hypothesis-testing framework (Experiments, Observations, Backtests) and screenshot management are excellent and market-independent.
...
## 7. Binary Options Requirements

To function as a dedicated Binary Options OS, the system must natively support and prioritize these concepts:
...
*   **Stake & Payout:** Binary returns are a fixed percentage (`payout`) of the risked amount (`stake`). *Currently exists but is hardcoded in `TPL-Trade.md` (Stake: 10, Payout: 85) instead of being dynamic.*
*   **OTC vs Live Markets:** Platforms like Quotex heavily feature algorithmic OTC markets. *The system already partially supports this via `asset-type: OTC` and `market-behavior` (Normal/Reversal), which is excellent.*
...
1.  **Hardcoded Values in Trade Template:** `TPL-Trade.md` hardcodes `expiry: "5m"`, `payout: 85`, and `stake: 10`. This creates inaccurate data if the user doesn't manually edit the YAML every time, which breaks the mobile-first UX principle.
2.  **Dashboard Fragmentation:** Having a `Dashboard - Trading.md` and a `Dashboard - Binary Options.md` creates confusion. If the vault is exclusively for BO, the "Generic" trading dashboard is redundant and confusing.
3.  **Net P/L Calculation Complexity:** The Dataview queries use complex inline logic to calculate BO profits: `choice(r.result = "WIN", r.stake * (r.payout/100), choice(r.result = "LOSS", -r.stake, 0))`. This is repeated across multiple dashboards, violating DRY (Don't Repeat Yourself). It would be safer if `net-pl` was calculated and stored at note creation or via a dedicated DataviewJS script.
...
*   **`Dashboard - Binary Options.md`:** This becomes redundant once the core system is fully BO-native.
...
The target architecture will be a **pure Binary Options OS**.

**Core Paradigm Shift:**
The system will no longer view Binary Options as a "mode" but as the fundamental reality of the database.

**Key Changes:**
1.  **Unified Trading Dashboard:** `Dashboard - Trading.md` will become the central hub, incorporating the Expiry, Direction, and OTC Behavior tables currently isolated in the BO dashboard. `Dashboard - Binary Options.md` will be deprecated.
...
*   Migrate all valuable Dataview queries from `Dashboard - Binary Options.md` into `Dashboard - Trading.md`.
...
*   Delete `Dashboard - Binary Options.md`.

**Phase 4: Historical Data Cleanup (High Risk)**
*   Run a script or manual search-and-replace across `01-Journal/Trades/` to remove the `trade-type` property from existing notes.
*   Ensure existing BO trades have correct `stake` and `payout` values populated, overriding the hardcoded defaults if necessary.
...
*   `00-OS/Dashboard - Binary Options.md` (To be DELETED)
*   Existing Trade Notes in `01-Journal/Trades/` (to remove `trade-type` and fix hardcoded stake/payout).
...
*   `03-Research/*` (Research methodologies are sound).
...
1.  **Historical Data Migration:** Do existing trades in `01-Journal/Trades/` hold real historical data that needs precise recalculation of `net-pl` based on accurate historical `stake` and `payout` values, or can we simply batch-remove the `trade-type` property and leave the rest as-is?
2.  **`net-pl` Calculation:** Should `net-pl` be calculated strictly dynamically via Dataview at dashboard runtime, or statically written to the YAML frontmatter at the time of note creation (which requires complex Templater JS logic if the `result` is added later)?
...
The `legendjournal` repository is an exceptionally well-engineered personal operating system. The migration to a pure Binary Options architecture is entirely feasible and highly recommended to reduce cognitive load and data-entry friction.
